...
import ru.baltinov.geometry.variablePoint.PointXYZ;
...
public class Line<T extends PointXY>  implements Cloneable{
  ...
  private static <T extends PointXY> T createCopy(T point) {
    if (point instanceof PointXYZ) {
      PointXYZ xyz = (PointXYZ) point;
      return (T) new PointXYZ(xyz.getX(), xyz.getY(), xyz.getZ());
    } else {
  ...
  public double length() {
    if (PStart instanceof PointXYZ && PEnd instanceof PointXYZ) {
      PointXYZ start3D = (PointXYZ) PStart;
      PointXYZ end3D = (PointXYZ) PEnd;
      int dx = end3D.getX() - start3D.getX();
      int dy = end3D.getY() - start3D.getY();
      int dz = end3D.getZ() - start3D.getZ();
